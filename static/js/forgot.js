...
form.addEventListener('submit', (event) =>{
    ...
    fetch("/forgotpassword",{method:'POST',headers: {'Content-Type': 'application/json'}, body : JSON.stringify(data)}).then(response => response.json()).then(data =>{
        if(data.sent == true){
            forgotsection.innerHTML = "<h3>Thank's for your interest, The mail has been sent to your email box</h3>"
        }
        else{
            forgotsection.innerHTML = "<h3>An error hmm...., Try again later</h3>"
        }
    })
})